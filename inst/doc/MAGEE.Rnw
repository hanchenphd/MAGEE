...
@
In this example, the first argument in \textbf{glmm.gei.meta} is tab or space delimited plain text files (or compressed files that can be recognized by the R function read.table) with at least the following columns: SNPID, CHR, POS, Non\textunderscore Effect\textunderscore Allele, Effect\textunderscore Allele, N\textunderscore Sample, AF, Beta\textunderscore Marginal, SE\textunderscore Beta\textunderscore Marginal, P\textunderscore Value\textunderscore Marginal, Beta\textunderscore G, \\ Beta\textunderscore G\textunderscore sex, SE\textunderscore Beta\textunderscore G, SE\textunderscore Beta\textunderscore G\textunderscore sex, Cov\textunderscore Beta\textunderscore G\textunderscore G-sex, P\textunderscore Value\textunderscore Interaction, \\P\textunderscore Value\textunderscore Joint. Generally, if each study performs score tests using genotypes in PLINK binary PED format or GDS format, the score test output from glmm.score can be directly used as input files. The argument "interaction" can be either a character vector indicating one or multiple environmental factors, or a numerical vector indicating the column numbers for the environmental factors in the covariate matrix. The argument "outfile" is the name of the output file.
...
@
The first argument in \textbf{MAGEE.meta} is a vector of intermediate files' prefix with length equal to the number of studies. The argument "group.file" is the name of the group definition file. The users can choose one or more test types as "IV", "IF", "JV", "JF", and "JD" in the "tests" argument. 
The \textbf{MAGEE.meta} function returns a data.frame object for both GDS and BGEN genotype file inputs. Below are examples for the first 5 rows of the example output: 
\begin{verbatim}
group    n.variants   
Set1     20           
Set2     20          
...
0.2882450    0.57320809    0.5751443
\end{verbatim}
The first column contains the group name (group) followed by the number of variants in the group in the second column (n.variants). The results are included in the next 7 columns: \textit{P} values for the MV test (MV.pval), MF test (MF.pval), IV test (IV.pval), IF test (IF.pval), JV test (JV.pval), JF test (JF.pval), and JD test (JD.pval).
...
If you use a GDS genotype file "geno.gds", here are the header and the first 5 rows of the example output "outfile.txt" using the default settings from \textbf{glmm.gei}:
\begin{verbatim}
...
\end{verbatim}
The first 5 columns are extracted from the GDS file: SNP ("annotation/id"), CHR ("chromosome"), POS ("position"), reference and alternate alleles ("allele"). Results are included in 13 columns for the ALT allele: the sample size N\textunderscore Sample (with non-missing genotypes), the allele frequency (AF), the number of non-missing samples  (N\textunderscore sex\textunderscore 0, and N\textunderscore sex\textunderscore 1) and allele frequency of the effect allele (AF\textunderscore sex\textunderscore 0 and AF\textunderscore sex\textunderscore 1) for each combination of strata for all of the catgorical exposure or interaction covariate, the coefficient estimate for the marginal genetic effect (Beta\textunderscore Marginal), the standard error (SE) of the marginal genetic effect (SE\textunderscore Beta\textunderscore Marginal), the coefficient estimate for the interaction term sex (Beta\textunderscore  G-sex), the model-based SE associated with any GxE term (SE\textunderscore Beta\textunderscore G-sex), the marginal effect score test \textit{P} value \textunderscore P\textunderscore Value\textunderscore Marginal, the gene-environment interaction test \textit{P} value P\textunderscore Value\textunderscore Interaction, and the joint test \textit{P} value P\textunderscore Value\textunderscore Joint.


If you use a BGEN genotype file "geno.bgen", here are the header and the first 5 rows of the example output "outfile.txt" using the default settings from \textbf{glmm.gei}:
\begin{verbatim}
...
\end{verbatim}
The first 6 columns are copied from the BGEN file: the SNP, RSID, chromosome CHR, physical position POS, and reference and alternate alleles ("allele"). Results are included in 13 columns for the second allele in the BGEN file: the sample size N\textunderscore Sample (with non-missing genotypes), the allele frequency (AF), the number of non-missing samples (N\textunderscore sex\textunderscore 0, N\textunderscore sex\textunderscore 1) and allele frequency of the effect allele (AF\textunderscore sex\textunderscore 0, AF\textunderscore sex\textunderscore 1) for each combination of strata for all of the catgorical exposure or interaction covariate, the coefficient estimate for the marginal genetic effect (Beta\textunderscore Marginal), the SE of the marginal genetic effect (SE\textunderscore Beta\textunderscore Marginal), the coefficient estimate for the interaction term sex (Beta\textunderscore G-sex), the model-based SE associated with GxE term (SE\textunderscore Beta\textunderscore G-sex), the marginal effect score test \textit{P} value P\textunderscore Value\textunderscore Marginal, the gene-environment interaction test \textit{P} value P\textunderscore Value\textunderscore Interaction, and the joint test \textit{P} value P\textunderscore Value\textunderscore Joint.
...
The meta-analysis function \textbf{glmm.gei.meta} generates a tab-delimited plain text output file. Here are the header and the first 5 rows of the example output from the meta-analysis:
\begin{verbatim}
...
...
\end{verbatim}
The first 3 columns are set by the function \textbf{glmm.gei.meta} to denote SNP name and alleles. The rest of the columns are: reference and alternate alleles ("allele"),  the sample size N\textunderscore Sample (with non-missing genotypes), the allele frequency (AF), the coefficient estimate for the marginal genetic effect (Beta\textunderscore Marginal), the SE of the marginal genetic effect (SE\textunderscore Beta\textunderscore Marginal), the coefficient estimate for the interaction term sex (Beta\textunderscore G-sex), the model-based SE associated with GxE term (SE\textunderscore Beta\textunderscore G-sex), the marginal effect score test \textit{P} value P\textunderscore Value\textunderscore Marginal, the gene-environment interaction test \textit{P} value P\textunderscore Value\textunderscore Interaction, and the joint test \textit{P} value P\textunderscore Value\textunderscore Joint.


In variant set tests \textbf{MAGEE}, if "meta.file.prefix" is specified, space-delimited intermediate files for single variant scores and binary intermediate files for covariance matrices will be generated. Here are the header and the first 5 rows of the example intermediate file "MAGEE.meta.score.1":
\begin{verbatim}
...
...
\end{verbatim}
The first 5 columns are copied from the group definition file, indicating the variant set (group) id, variant chromosome, variant position, variant reference allele, variant alternate allele, respectively. Results are included in 6 columns: the sample size N (with non-missing genotypes), the genotype missing rate missrate, the alt allele frequency altfreq, the score statistic SCORE of alt allele, the score of the first environmental factor.
...
By default, genotypes are centered to the mean before the analysis in single variant tests \textbf{glmm.gei}. You can turn this feature off by specifying \begin{verbatim} geno.center = FALSE \end{verbatim} to use raw genotypes.

In \textbf{glmm.gei}, by default the interaction covariates (if any) are centered to have mean 0, but interaction exposures are not centered. This can be changed using the "covar.center" argument to "none" (no centering for any covariates) or "all" (centering all exposures and covariates to have mean 0). Generally, centering exposures and covariates to have mean 0 before creating interaction terms would make the genetic main effect easier to interpret. However, if a subsequent meta-analysis is expected, then the exposures of interest should not be centered because in that case the genetic main effect may have different interpretations across studies.
...
\subsection{Version 1.1.1 (April 12, 2022)}
\begin{enumerate}
\item Fixed bugs on inverse of singular matrix \textbf{glmm.gei}.
\end{enumerate}

\subsection{Version 1.2.0 (June 2, 2022)}
\begin{enumerate}
\item Added meta-analysis functions \textbf{glmm.gei.meta} and \textbf{MAGEE.meta}.
\end{enumerate}

\subsection{Version 1.3.0 (April 18, 2023)}
\begin{enumerate}
\item Check for system copies of zstd and libdeflate libraries.
\item Fixed a minor bug in reading the ID column for bgen sample files in \textbf{MAGEE} and \textbf{glmm.gei}.
\item Fixed a minor bug in calling the internal function \textbf{fix.dgesdd} in \textbf{glmm.gei}.
\item Replaced \textbf{read.table} by the more efficient function \textbf{data.table::fread}.
\item Implemented a new argument "AF.strata.range" to filter variants based on their environmental exposure stratum-specific coding allele frequencies in \textbf{MAGEE}.
\item Removed \textbf{context} in testthat tests.
\item Replaced \textbf{print} and \textbf{cat} in the code by \textbf{message} and \textbf{warning}.
...
\subsection{Version 1.4.0 (April 23, 2024)}
\begin{enumerate}
\item Added a new argument "covar.center" in \textbf{glmm.gei} to allow users to control whether and how the covariates (including exposures) should be centered.
\item Fixed a bug in reading BGEN files in \textbf{glmm.gei}. A sample file is expected to overwrite (possibly reshuffled) sample IDs in the BGEN file (thanks to: Simon Wiegrebe and Thomas Winkler).
\item Fixed a bug in reading BGEN files for longitudinal analysis in \textbf{glmm.gei}. The BGEN files can now include extra samples that are not included in the null model object (thanks to: Simon Wiegrebe and Thomas Winkler).
\item Fixed a bug in calculating allele frequencies for longitudinal analysis in \textbf{glmm.gei} (thanks to: Simon Wiegrebe and Thomas Winkler).
\item Fixed several minor bugs in the output for BGEN files in \textbf{glmm.gei}. An extra tab between "AF" and "Beta\_Marginal" was removed. The empty second row was removed. Three incorrect columns for continuous exposures were removed (thanks to: Simon Wiegrebe and Thomas Winkler).
\item Improved error control for negative variances in \textbf{glmm.gei}.
\item Added minor allele count and Rsq filters in \textbf{glmm.gei} (thanks to: Simon Wiegrebe and Thomas Winkler).
\item Added support for imputed dosage GDS files (in the node annotation/format/DS/data).
\item Fixed a minor bug in output header names in \textbf{glmm.gei.meta} (thanks to: Christopher Bryan).
\end{enumerate}

\subsection{Version 1.4.1 (April 28, 2024)}
\begin{enumerate}
\item Removed "ARMA\_DONT\_PRINT\_ERRORS" in fitglmm.cpp (which was deprecated in RcppArmadillo version 0.11.2.1.0) and fixed minor bugs identified in the tests for link-time optimization type mismatches (thanks to: Prof. Brian Ripley).
\end{enumerate}

\subsection{Version 1.4.2 (July 26, 2024)}
\begin{enumerate}
\item Added a new argument "cat.threshold" in \textbf{glmm.gei} to allow users to control the cut-off threshold for interaction terms and interaction covariates to be treated as categorical (thanks to: Simon Wiegrebe).
\end{enumerate}

\subsection{Version 1.4.3 (July 12, 2025)}
\begin{enumerate}
\item Added a new argument "covar.center" in \textbf{MAGEE} and \textbf{MAGEE.prep} to allow users to control whether and how the covariates (including exposures) should be centered.
\item Changed the default "nperbatch" in \textbf{glmm.gei} from 100 to 1 to avoid linear algebra numeric issues found in the tests.
\end{enumerate}

\subsection{Version 1.4.4 (October 20, 2025)}
\begin{enumerate}
\item Fixed a minor bug in fitglmm.cpp due to deprecated "arma::is\_finite" in Armadillo 15.
\item Fixed minor issues in the vignette (thanks to: Dan Gealow).
\end{enumerate}

\section{Contact}
Please refer to the R help document of \textit{MAGEE} for specific questions about each function. For comments, suggestions, bug reports and questions, please contact Han Chen (hanchenphd@gmail.com). For bug reports, please include an example to reproduce the problem without having to access your confidential data.
